...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if (!$fromBranch || !$toBranch || !$itemId || $quantity <= 0 || $fromBranch === $toBranch) {
            ...
            if (!$item || $item['current_stock'] < $quantity) {
                ...
                try {
                    ...
                    $pdo->commit();
                    $notifyUsers = $pdo->prepare("SELECT id FROM users WHERE is_active = 1 AND role_id IN (1,2) AND branch_id IN (?, ?)");
                    $notifyUsers->execute([$fromBranch, $toBranch]);
                    foreach ($notifyUsers->fetchAll() as $notifyUser) {
                        $pdo->prepare("INSERT INTO notifications (user_id, type, title, message) VALUES (?, 'transfer', 'New transfer request', ?)")
        ...
        $status = $_POST['status'] ?? '';
        if ($canManage && in_array($status, ['Approved', 'Dispatched', 'Received', 'Rejected', 'Cancelled'])) {
            ...
            if ($transfer) {
                if ($status === 'Approved') {
                    ...
                    $pdo->prepare("INSERT INTO notifications (user_id, type, title, message) VALUES (?, 'transfer', 'Transfer approved', ?)")
                        ->execute([$transfer['requested_by'], 'Transfer ' . $transfer['transfer_code'] . ' has been approved.']);
                    ...
                    try {
                        ...
                        $pdo->prepare("INSERT INTO notifications (user_id, type, title, message) VALUES (?, 'transfer', 'Transfer received', ?)")
                            ->execute([$transfer['requested_by'], 'Transfer ' . $transfer['transfer_code'] . ' has been received.']);
                        $pdo->commit();
                    } catch (Exception $e) {
                        $pdo->rollBack();
                    ...
                    $pdo->prepare("UPDATE transfers SET status = 'Rejected' WHERE id = ?")
                        ->execute([$transferId]);
                } elseif ($status === 'Cancelled') {
                    $pdo->prepare("UPDATE transfers SET status = 'Cancelled' WHERE id = ?")
                        ->execute([$transferId]);
                }
            ...
            <tr>
                ...
                <td>
                    <div class="action-btns">
                        <?php if ($canManage && $row['status'] === 'Requested'): ?>
                        ...
                        <?php endif; ?>
                        <?php if ($canManage && $row['status'] === 'Approved'): ?>
                        <form method="POST" style="display:inline">
                            <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                            <input type="hidden" name="action" value="update_status">
                            <input type="hidden" name="transfer_id" value="<?= $row['id'] ?>">
                            <input type="hidden" name="status" value="Dispatched">
                            <button type="submit" class="btn btn-outline">Dispatch</button>
                        </form>
                        <?php endif; ?>
                        <?php if ($canManage && $row['status'] === 'Dispatched'): ?>
                        <form method="POST" style="display:inline">
                            <input type="hidden" name="csrf_token" value="<?= csrfToken() ?>">
                            <input type="hidden" name="action" value="update_status">
                            <input type="hidden" name="transfer_id" value="<?= $row['id'] ?>">
