        ...
        if (!$fullName||!$roleId||!$branchId) { setFlash('error','Full name, role, and campus are required.'); }
            ...
            if ($action==='add') {
                    ...
                    try {
                        ...
                        if (($e->errorInfo[1] ?? null) === 1062) {
                            ...
                            if (stripos($msg, 'email') !== false) {
                                setFlash('error', "A user with the email '$email' already exists. Please use a different email address.");
                                header('Location: users.php'); exit;
                            }
                            // Duplicate username — retry with a generated one
                            $username = generateUsername($fullName, $branchId);
                            try {
                                $pdo->prepare("INSERT INTO users (branch_id,section_id,department_id,role_id,full_name,email,username,password,phone,is_active,profile_photo,must_change_password,password_changed_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,1,NULL)")->execute([$branchId,$sectionId?:null,$departmentId?:null,$roleId,$fullName,$email,$username,$hash,$phone,$status,null]);
                            } catch (PDOException $e2) {
                                if (($e2->errorInfo[1] ?? null) === 1062 && stripos($e2->getMessage(), 'email') !== false) {
                                    setFlash('error', "A user with the email '$email' already exists. Please use a different email address.");
                                    header('Location: users.php'); exit;
                                }
                                throw $e2;
                ...
                try {
                    ...
                    if (($e->errorInfo[1] ?? null) === 1062 && stripos($e->getMessage(), 'email') !== false) {
                        setFlash('error', "A user with the email '$email' already exists. Please use a different email address.");
                        header('Location: users.php'); exit;
                    }
                    throw $e;
...
if ($searchQuery !== '') {
    $whereConditions[] = "(u.full_name LIKE ? OR u.username LIKE ? OR u.email LIKE ? OR u.phone LIKE ?)";
...
if ($statusFilter !== 'all') {
    if ($statusFilter === 'active') {
        $whereConditions[] = "u.is_active = 1 AND (u.must_change_password = 0 OR u.must_change_password IS NULL)";
    } elseif ($statusFilter === 'inactive') {
        $whereConditions[] = "u.is_active = 0";
    } elseif ($statusFilter === 'pending') {
        $whereConditions[] = "u.is_active = 1 AND u.must_change_password = 1";
    }
}
...
$usersStmt = $pdo->prepare("
    SELECT u.*, r.name AS role_name, b.name AS branch_name, s.name AS section_name, d.name AS department_name 
    FROM users u 
...
<div class="filter-bar">
    <form method="GET" class="filter-form" id="userFilterForm">
        <div class="filter-group">
            <input type="text" name="q" placeholder="Search users..." value="<?= clean($searchQuery) ?>" class="form-control" style="width: 250px;">
        </div>
        <div class="filter-group">
            <select name="role" onchange="this.form.submit()">
                <option value="0">All Roles</option>
                <?php foreach ($roles as $r): ?>
                <option value="<?= $r['id'] ?>" <?= $roleFilter == $r['id'] ? 'selected' : '' ?>><?= clean($r['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="filter-group">
            <select name="branch" onchange="this.form.submit()">
                ...
                <option value="<?= $b['id'] ?>" <?= $branchFilter == $b['id'] ? 'selected' : '' ?>><?= clean($b['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        <div class="filter-group">
        ...
        <div class="filter-group">
            ...
            <?php if ($searchQuery !== '' || $roleFilter > 0 || $branchFilter > 0 || $statusFilter !== 'all'): ?>
            <a href="users.php" class="btn btn-outline" style="padding: 8px 12px; margin-left: 5px;">Clear</a>
            <?php endif; ?>
        </div>
    </form>
</div>
    ...
    <div class="card-body p0">
        ...
        <table class="data-table users-table">
            ...
            <?php foreach ($users as $i=>$u):
                ...
                $accountStatusClass = 'badge-success';
                if (!(int)$u['is_active']) {
                    $accountStatusLabel = 'Inactive';
                    $accountStatusClass = 'badge-danger';
                } elseif ((int)($u['must_change_password'] ?? 0) === 1) {
                    $accountStatusLabel = 'Pending';
                    $accountStatusClass = 'badge-warn';
                }
            ?>
            <tr>
                ...
                <td>
                    <span class="badge <?= $accountStatusClass ?>"><?= $accountStatusLabel ?></span>
                    <?php if ($accountStatusLabel === 'Pending'): ?>
            ...
            <div class="modal-body">
                <div class="form-grid-2">
                    ...
                    <div class="form-group"><label>Status</label>
                        ...
                        <small class="form-note">New or reset-password users show as Pending until they create their own password.</small>
                    </div>
