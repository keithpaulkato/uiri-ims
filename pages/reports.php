...
if ($searchQuery) {
    ...
    $searchSql = " AND (i.name LIKE $searchLike OR i.item_code LIKE $searchLike OR c.name LIKE $searchLike OR b.name LIKE $searchLike OR d.name LIKE $searchLike)";
}
...
$data = [];
$commonJoin = 'JOIN categories c ON i.category_id=c.id JOIN branches b ON i.branch_id=b.id LEFT JOIN departments d ON i.department_id=d.id';
$commonWhere = "WHERE i.is_active=1 $branchSql $catSql $departmentSql $supplierSql $sectionSql $stockStatusSql $searchSql";

if ($reportType === 'summary') {
    $data = $pdo->query("SELECT i.*, c.name AS category_name, b.name AS branch_name, (i.current_stock * i.unit_price) AS total_value FROM inventory_items i $commonJoin $commonWhere ORDER BY b.name, c.name, i.name")->fetchAll();
} elseif ($reportType === 'movement') {
    $sql = "SELECT t.*, i.name AS item_name, i.item_code, b.name AS branch_name, u.full_name AS user_name
    ...
    if ($searchQuery) {
        $sql .= " AND (i.name LIKE ? OR i.item_code LIKE ? OR b.name LIKE ? OR u.full_name LIKE ?)";
    ...
    $params = [];
    $reportWhere = "WHERE i.is_active=1 $branchSql $catSql $departmentSql $supplierSql $sectionSql $stockStatusSql $searchSql";
    ...
    $sql = "SELECT i.*, c.name AS category_name, b.name AS branch_name FROM inventory_items i $commonJoin WHERE i.is_active=1 AND i.current_stock <= i.minimum_stock $branchSql $catSql $departmentSql $supplierSql $sectionSql $searchSql ORDER BY i.current_stock ASC";
    $stmt = $pdo->prepare($sql);
    $stmt->execute();
    $data = $stmt->fetchAll();
}
...
<div class="card print-area">
    <div class="print-header">
        ...
        <div>
            ...
            <p>Generated: <?= date('d F Y, H:i') ?> by <?= clean($user['full_name']) ?></p>
        </div>
    </div>

    <?php if ($reportType==='summary' && $data): ?>
    <table class="data-table">
        <thead><tr><th>#</th><th>Item Code</th><th>Item Name</th><th>Category</th><?php if ($isAdmin): ?><th>Branch</th><?php endif; ?><th>Unit</th><th>Unit Price</th><th>Stock</th><th>Min Stock</th><th>Total Value</th><th>Status</th></tr></thead>
        <tbody>
        ...
        <tr><td><?= $i+1 ?></td><td><?= clean($row['item_code']) ?></td><td><?= clean($row['name']) ?></td><td><?= clean($row['category_name']) ?></td><?php if ($isAdmin): ?><td><?= clean($row['branch_name']) ?></td><?php endif; ?><td><?= clean($row['unit']) ?></td><td><?= ugx($row['unit_price']) ?></td><td><strong><?= number_format($row['current_stock']) ?></strong></td><td><?= $row['minimum_stock'] ?></td><td><?= ugx($row['total_value']) ?></td><td><span class="badge badge-<?= $ss ?>"><?= $ss==='danger'?'Out':($ss==='warn'?'Low':'OK') ?></span></td></tr>
        <?php endforeach; ?>
        </tbody>
        <tfoot><tr><td colspan="<?= $isAdmin?9:8 ?>"><strong>Grand Total Stock Value</strong></td><td colspan="2"><strong><?= ugx($grandTotal) ?></strong></td></tr></tfoot>
    </table>

    <?php elseif ($reportType==='movement' && $data): ?>
    <table class="data-table">
        <thead><tr><th>#</th><th>Date</th><th>Item</th><th>Branch</th><th>Type</th><th>Qty</th><th>Unit Price</th><th>Total</th><th>Reference</th><th>Recorded By</th></tr></thead>
        <tbody>
        <?php foreach ($data as $i=>$tx): $tc=['stock_in'=>'badge-success','stock_out'=>'badge-blue','adjustment'=>'badge-warn']; ?>
        <tr><td><?= $i+1 ?></td><td><?= date('d M Y',strtotime($tx['transaction_date'])) ?></td><td><span class="item-name"><?= clean($tx['item_name']) ?></span><span class="item-code"><?= clean($tx['item_code']) ?></span></td><td><?= clean($tx['branch_name']) ?></td><td><span class="badge <?= $tc[$tx['transaction_type']]??'badge-blue' ?>"><?= str_replace('_',' ',ucfirst($tx['transaction_type'])) ?></span></td><td><?= number_format($tx['quantity']) ?></td><td><?= $tx['unit_price']>0?ugx($tx['unit_price']):'—' ?></td><td><?= $tx['unit_price']>0?ugx($tx['quantity']*$tx['unit_price']):'—' ?></td><td><?= clean($tx['reference_number']?:'—') ?></td><td><?= clean($tx['user_name']) ?></td></tr>
        <?php endforeach; ?>
        </tbody>
    </table>
    ...
    <table class="data-table">
        <thead><tr><th>#</th><th>Item Code</th><th>Item Name</th><th>Category</th><?php if ($isAdmin): ?><th>Branch</th><?php endif; ?><th>Current Stock</th><th>Min Stock</th><th>Deficit</th><th>Status</th></tr></thead>
        <tbody>
        <?php foreach ($data as $i=>$row): $deficit=max(0,$row['minimum_stock']-$row['current_stock']); ?>
        <tr><td><?= $i+1 ?></td><td><?= clean($row['item_code']) ?></td><td><?= clean($row['name']) ?></td><td><?= clean($row['category_name']) ?></td><?php if ($isAdmin): ?><td><?= clean($row['branch_name']) ?></td><?php endif; ?><td><span class="badge <?= $row['current_stock']==0?'badge-danger':'badge-warn' ?>"><?= $row['current_stock'] ?></span></td><td><?= $row['minimum_stock'] ?></td><td><?= $deficit ?></td><td><?= $row['current_stock']==0?'<span class="badge badge-danger">Out of Stock</span>':'<span class="badge badge-warn">Low Stock</span>' ?></td></tr>
        <?php endforeach; ?>
        </tbody>
    </table>
